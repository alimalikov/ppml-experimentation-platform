...
class MacroAvgF1Metric(MetricPlugin):
    """Macro-Averaged F1 Score metric for multi-class classification tasks."""
    
    def __init__(self):
        super().__init__()
        self._name = "Macro-Averaged F1 Score"
        self._description = "Unweighted mean of F1 scores for each class. Treats all classes equally regardless of their frequency."
    ...
    def calculate(self, y_true: np.ndarray, y_pred: np.ndarray, y_proba: Optional[np.ndarray] = None, **kwargs) -> float:
        try:
            ...
            raise ValueError(f"Error calculating Macro-Averaged F1 Score: {str(e)}")
