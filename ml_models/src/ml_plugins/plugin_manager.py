...
class MLPluginManager:
    ...
    def discover_plugins(self):
        """Public method to discover and load plugins (alias for reload)."""
        self.reload_plugins()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        """
        # Fix the statistics method to work with the actual plugin structure
        return {
            'total_plugins': len(self._loaded_plugins),
            'categories': dict(self._plugin_categories),
            'plugins_by_category': {cat: len(plugins) for cat, plugins in self._plugin_categories.items()},
