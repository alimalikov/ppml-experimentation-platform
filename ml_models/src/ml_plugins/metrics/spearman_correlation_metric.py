...
class SpearmanCorrelationMetric(MetricPlugin):
    """Spearman Rank Correlation Coefficient metric for regression tasks."""
    
    def __init__(self):
        super().__init__()
        self._name = "Spearman Rank Correlation"
        self._description = "Measures monotonic correlation between predicted and actual values. More robust to outliers than Pearson."
    ...
    def calculate(self, y_true: np.ndarray, y_pred: np.ndarray, y_proba: Optional[np.ndarray] = None, **kwargs) -> float:
        try:
            # Handle edge cases
            if len(y_true) < 2:
                raise ValueError("Spearman correlation requires at least 2 data points")
            ...
            raise ValueError(f"Error calculating Spearman correlation: {str(e)}")
...
def get_metric_plugin() -> MetricPlugin:
    return SpearmanCorrelationMetric()
