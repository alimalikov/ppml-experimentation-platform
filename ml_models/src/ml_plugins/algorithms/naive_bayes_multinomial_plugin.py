...
class MultinomialNaiveBayesPlugin(BaseEstimator, ClassifierMixin, MLPlugin):
    ...
    def get_algorithm_specific_metrics(self, y_true: Optional[np.ndarray] = None, y_pred: Optional[np.ndarray] = None, y_proba: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        else: # If no vectorizer and no direct vocabulary, use n_features_in_ from model
            if hasattr(self.model_, 'n_features_in_'):
                 metrics[f"{prefix}vocabulary_size_or_features"] = self.model_.n_features_in_
        ...
        if self.vectorizer_ and hasattr(self.vectorizer_, 'get_feature_names_out'):
            original_num_features = len(self.vectorizer_.get_feature_names_out())
            metrics[f"{prefix}num_features_after_vectorization"] = original_num_features
        elif self.feature_names_ and not self.feature_selector_: # Pre-tokenized, no selection
            original_num_features = len(self.feature_names_)

        if self.feature_selector_ and hasattr(self.feature_selector_, 'get_support'):
            if original_num_features is not None:
                 metrics[f"{prefix}num_features_before_selection"] = original_num_features
            metrics[f"{prefix}num_features_after_selection"] = int(np.sum(self.feature_selector_.get_support()))
        elif original_num_features is not None and not self.feature_selector_: # No selection applied
             metrics[f"{prefix}num_features_total"] = original_num_features
        ...
        if self.class_log_prior_ is not None:
            ...
            # Individual class priors can be added if needed, but might make dict large
        ...
        if self.feature_importance_scores_ is not None and len(self.feature_importance_scores_) > 0:
        ...
        if y_true is not None and y_proba is not None and self.label_encoder_ is not None and self.classes_ is not None:
            try:
                ...
                # Ensure y_true_encoded is within bounds for y_proba indexing
                ...
                clipped_proba = np.clip(y_proba, 1e-15, 1 - 1e-15) # Avoid log(0)
                ...
                # Ensure minlength matches the number of classes in the model for bincount
                class_counts = np.bincount(y_true_encoded, minlength=n_classes_model)
                
                # Handle cases where some classes might not be in y_true but are in the model
                ...
                ll_null = 0
                for k_idx in range(n_classes_model): # Iterate up to n_classes_model
                ...
                if ll_null == 0: # Avoid division by zero if null model has zero likelihood (e.g. one class in y_true)
                    metrics[f"{prefix}mcfaddens_pseudo_r2"] = 1.0 if ll_model == 0 else 0.0 # Perfect fit or undefined
