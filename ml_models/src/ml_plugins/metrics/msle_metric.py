...
class MSLEMetric(MetricPlugin):
    """Mean Squared Log Error metric for regression tasks with exponential growth patterns."""
    
    def __init__(self):
        super().__init__()
        self._name = "Mean Squared Log Error"
        self._description = "Average of squared differences between log-transformed predicted and actual values. Best for exponential growth patterns."
        self._category = "Regression"
        self._supports_classification = False
        self._supports_regression = True
        self._requires_probabilities = False
        self._higher_is_better = False  # Lower MSLE is better
    ...
    def calculate(self, y_true: np.ndarray, y_pred: np.ndarray, y_proba: Optional[np.ndarray] = None, **kwargs) -> float:
        try:
            ...
            if np.any(y_true < 0) or np.any(y_pred < 0):
                raise ValueError("MSLE requires non-negative values for log transformation")
            
            return float(mean_squared_log_error(y_true, y_pred))
        except Exception as e:
            raise ValueError(f"Error calculating MSLE: {str(e)}")
