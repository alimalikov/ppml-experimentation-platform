...
# ========================= USER DATASET UPLOAD SECTION ========================= #
st.subheader("📁 Upload Your Own Dataset")
st.markdown("""Upload your own CSV or Excel file to test anonymization techniques on your data. 
...
with col1:
    ...
    uploaded_file = st.file_uploader(
        ...
        help="Upload a CSV or Excel file containing your data for anonymization"
    )
...
st.sidebar.markdown("*Try the platform with built-in datasets or upload your own data above*")
st.sidebar.info("💡 **Tip:** Upload your own CSV/Excel file in the main area, or use sample datasets below for testing.")
...
if uploaded_file is None and \
   ...
   st.session_state.last_uploader_key_persisted == uploaded_file_key:
    ...
    # Determine if this is user data or sample data
    is_sample_data = any(sample_key in file_name_persisted for sample_key in [
        'iris', 'wine', 'breast_cancer', 'diabetes', 'digits', 'california_housing',
        'boston_synthetic', 'credit_approval_synthetic', 'marketing_campaign_synthetic',
        'flower_species_synthetic', 'music_genre_synthetic', 'stock_prices_synthetic',
        'titanic', 'heart_disease_cleveland', 'auto_mpg', 'adult_income',
        'customer_data', 'employee_records', 'medical_records', 'financial_transactions',
        'survey_responses', 'clinical_trials'
    ])
    
    if is_sample_data:
        st.info(f"🎲 Using sample dataset: {file_name_persisted}")
    else:
        st.info(f"📁 Using your uploaded data: {file_name_persisted}")
    try:
    ...
    try:
        ...
        if df_raw is not None:
             st.success(f"✅ Successfully uploaded '{uploaded_file.name}' with {len(df_raw):,} rows and {len(df_raw.columns)} columns!")
             
             # Show data type summary
             numeric_cols = len(df_raw.select_dtypes(include='number').columns)
             text_cols = len(df_raw.select_dtypes(include='object').columns)
             datetime_cols = len(df_raw.select_dtypes(include='datetime').columns)
...
if df_raw is not None:
    ...
    try:
        ...
        current_file_name = st.session_state.persisted_uploaded_file_name or "Unknown"
        is_user_data = not any(sample_key in current_file_name for sample_key in [
            'iris', 'wine', 'breast_cancer', 'diabetes', 'digits', 'california_housing',
            'boston_synthetic', 'credit_approval_synthetic', 'marketing_campaign_synthetic',
            'flower_species_synthetic', 'music_genre_synthetic', 'stock_prices_synthetic',
            'titanic', 'heart_disease_cleveland', 'auto_mpg', 'adult_income',
        ...
        data_source_icon = "📁" if is_user_data else "🎲"
        data_source_text = "Your Dataset" if is_user_data else "Sample Dataset"
        st.subheader(f"{data_source_icon} {data_source_text} Preview (First 5 rows)")
        ...
        if df_anonymized_current_run is not None and not df_anonymized_current_run.empty:
            ...
            # Download button for XLSX with better labeling
            base_filename = current_file_name_for_download.split('.')[0] if current_file_name_for_download else "data"
            download_filename = f"anonymized_{plugin_key_prefix_base}_{base_filename}.xlsx"
            
            st.download_button(
                label=f"📥 Download Anonymized Data ({selected_technique_name})", 
                data=xlsx_anonymized, 
                file_name=download_filename, 
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key=f"download_{plugin_key_prefix}",
                help=f"Download your anonymized dataset as an Excel file: {download_filename}"
            )
...
# This 'else' corresponds to 'if df_raw is not None:'
    ...
    st.info("📤 **Upload your dataset above or select a sample dataset from the sidebar to get started.**")
    
    # Show helpful tips when no data is loaded
    with st.expander("💡 Getting Started Tips", expanded=True):
        st.markdown("""
        **To begin anonymizing your data:**
        
        1. **Upload Your Own Data:** Use the file uploader above to upload a CSV or Excel file
        2. **Try Sample Data:** Select from built-in datasets in the sidebar for testing
        3. **Choose Technique:** Pick an anonymization technique from the sidebar
        4. **Configure & Apply:** Set parameters and apply the anonymization
        ...
        **Data Requirements:**
        - Files should have column headers in the first row
        - Maximum recommended file size: 50MB for optimal performance
        - Ensure your data doesn't contain sensitive information you don't want processed
        """)
        ...
        if st.sidebar.button(load_button_text, type="secondary", key="load_sample_btn"):
            with st.spinner(f"Loading {selected_dataset} ({dataset_size_key} size)..."):
                ...
                if sample_df is not None:
                    # Clear any existing data (both user uploads and previous samples)
                    st.session_state.persisted_uploaded_file_bytes = None
                    st.session_state.persisted_uploaded_file_name = None
                    st.session_state.last_uploader_key_persisted = None
                    st.session_state.df_anonymized_data = None
                    ...
                    # Clear previous sample dataset info
                    if hasattr(st.session_state, 'sample_dataset_info'):
                        st.session_state.sample_dataset_info = None
                    if hasattr(st.session_state, 'loaded_sample_dataset_name'):
                        st.session_state.loaded_sample_dataset_name = None
                    ...
                    # Clear any previous sample dataset info when loading new sample
                    st.session_state.sample_dataset_info = None
                    st.session_state.loaded_sample_dataset_name = None
...
if (hasattr(st.session_state, 'sample_dataset_info') and 
    ...
    if (st.session_state.persisted_uploaded_file_bytes and 
        ...
        # Check if it's user data or sample data
        is_user_data = not any(sample_key in st.session_state.persisted_uploaded_file_name for sample_key in [
            'iris', 'wine', 'breast_cancer', 'diabetes', 'digits', 'california_housing',
            'boston_synthetic', 'credit_approval_synthetic', 'marketing_campaign_synthetic',
            'flower_species_synthetic', 'music_genre_synthetic', 'stock_prices_synthetic',
            'titanic', 'heart_disease_cleveland', 'auto_mpg', 'adult_income',
        ...
        download_label = "📥 Download Your Dataset" if is_user_data else "📥 Download Sample Dataset"
        ...
        st.sidebar.download_button(
            ...
            mime="text/csv",
            key="download_original_dataset",
            help="Download the currently loaded dataset for future use",
